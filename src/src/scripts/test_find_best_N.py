...
from spr_functions.other_recs_with_diff_metrics import OtherRecsALSRecommender, OtherRecsALSRecommenderKMeans, OtherRecsALSRecommenderKNN
...
def create_parser():
    ...
    parser.add_argument("--n_users", default=5000, type=int, required=False)
...
if __name__ == "__main__":
    ...
    for n in n_clusters:
        ...
        train_data = loader.load_data('train')
        vad_data_tr, vad_data_te = loader.load_data('validation')
        model = OtherRecsALSRecommenderKMeans(train_data=train_data,
                                              train_matrix=train_matrix,
                                              vad_data_te=vad_data_te,
                                              vad_data_tr=vad_data_tr,
                                              loader=loader)
        
        hyperparams = {'metric': 'cosine', 'alpha': 7, 'factors': 42, 'n_clusters' : n}   
        ...
        print("=" * 89)
        print('n_clusters:', hyperparams['n_clusters'])
