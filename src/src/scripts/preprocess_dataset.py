...
from settings import DIR_TO_NETFLIX
...
class DataLoader:
    ...
    def _load_train_data(self):
        ...
        data = sparse.csr_matrix(
            (np.ones_like(rows), (rows, cols)), dtype="float64", shape=(n_users, self.n_items),
        )
        return data
...
def split_train_test_proportion(data, test_prop=0.2):
    ...
    for _, group in data_grouped_by_user:
        ...
        if n_items_u >= 5:
            ...
            idx[
                np.random.choice(n_items_u, size=int(test_prop * n_items_u), replace=False).astype(
                    "int64"
                )
            ] = True
...
def create_parser():
    ...
    parser.add_argument("--data", type=str, required=False, default=DIR_TO_ML1M)
    parser.add_argument("--nrows", type=int, default=None)
    parser.add_argument("--data_name", type=str, default='M')
    return parser
...
if __name__ == "__main__":
    ...
    if args.data_name == 'M' :
        raw_data = pd.read_csv(
                os.path.join(args.data, "ratings.dat"), header=None, sep="::", nrows=args.nrows
        )
